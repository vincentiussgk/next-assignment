...
import {
  IUsersFormState,
  IUsersValidationState,
} from "@/types/inputInterfaces";
...
import SelectInput from "./admin/components/SelectInput";
import Button from "./admin/components/Button";
import { Role } from "@/types/dropdownValues";
import Link from "next/link";
import { manipWithFetch } from "@/lib/fetch";
import { toast } from "react-toastify";
import { membershipLabels } from "@/types/roleStyles";
...
const Profile = () => {
  ...
  const handleSubmit = async (e: FormEventHandler<HTMLInputElement>) => {
    e.preventDefault();

    await manipWithFetch(`users/${currentUser?.id}`, userFormState, "PATCH");
  ...
  return (
    <div className="bg-white m-[50px] p-[50px] rounded-[16px]">
        ...
        <div className={`flex w-full gap-5 justify-between gap-x-[15px]`}>
          ...
          <SelectInput
            ...
            label={
              <div>
                Membership (
                <Link className={`text-[#B0C4DE]`} href="/membership">
                  Upgrade
                </Link>
                )
              </div>
            ...
            options={Object.values(membershipLabels)}
